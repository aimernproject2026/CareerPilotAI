...
function Step1SetUp({ onStart, preferredGender, setPreferredGender }) {
    ...
    const handleUploadResume = async () => {
        ...
        try {
            ...
            setResumeQuality(result.data.resumeQuality || "");
            setResumeFeedback(result.data.resumeFeedback || "");
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            const result = await axios.post(ServerUrl + "/api/interview/generate-questions", { role, experience, mode, resumeText, projects, skills, companyName, jobDescription }, { withCredentials: true })
            console.log(result.data)
            if (userData) {
                ...
                <motion.div
                    ...
                    <div className='space-y-6'>
                        ...
                        <div className='flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl p-3'>
                            <span className='text-md text-gray-600 font-medium'>Interviewer :</span>
                            <div className='flex gap-2 ml-auto'>
                                <button
                                    type='button'
                                    onClick={() => setPreferredGender("female")}
                                    className={`px-4 py-1.5 rounded-full cursor-pointer text-sm font-medium transition ${preferredGender === "female" ? "bg-green-600 text-white" : "bg-white border border-gray-300 text-gray-600"}`}>
                                    👩 Female
                                </button>
                                <button
                                    type='button'
                                    onClick={() => setPreferredGender("male")}
                                    className={`px-4 py-1.5 rounded-full cursor-pointer text-sm font-medium transition ${preferredGender === "male" ? "bg-green-600 text-white" : "bg-white border border-gray-300 text-gray-600"}`}>
                                    👨 Male
                                </button>
                            </div>
                        </div>
                        ...
                        {/* Job Description */}
                        <textarea
                            placeholder={mode === "Technical" ? "Strong Topic (e.g. React JS, DBMS, OS) — Optional" : "Paste Job Description here (Optional but recommended)"}
                            rows={4}
                                ...
                                {resumeQuality && (
                                    <div className='border-t border-gray-200 pt-3 space-y-1'>
                                        <div className='flex items-center gap-2'>
                                            ...
                                            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${resumeQuality === "Good" ? "bg-green-100 text-green-700" :
                                                    resumeQuality === "Moderate" ? "bg-yellow-100 text-yellow-700" :
                                                        "bg-red-100 text-red-600"
                                                }`}>{resumeQuality}</span>
                                        </div>
                                        {resumeFeedback && (
                                            <p className='text-sm text-gray-500 leading-relaxed'>{resumeFeedback}</p>
                                        )}
                                    </div>
                                )}
